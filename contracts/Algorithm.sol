...
contract Algorithm {
  ...
  function roulette(uint8 hitNumber, uint16 totalIssuance) public view returns (uint8 winner){
    uint16 _rangeMin;
    uint16 _rangeMax;
    uint16 _percentage;
    uint8 k;
    ...
    for (k = 0; k < 10; k++) {
      _percentage = (userMapping[k].purchasedTokenAmount / totalIssuance) * 100;
